...
console.log(document.getElementById('place_order').innerHTML);
 ...
 price=[];
  $.ajax({
        type :"GET",
       url: "http://localhost:9000/EC/webapi/buyerService/allBuyerOrders",
       async: false,
       headers: {
        ...
        console.log(response);
             ...
             cart_product_list += '<div class="grid-container">';
              for(i in result)
              {
            	  ...
            	  if(result[i].buyer_id==document.getElementById('From').value)
            		  {
...
function cart_list(x){
	...
	cart_product_list+='<p>item_id:'+result[x].item_id+'</p>'
	cart_product_list+='<p>item_price:'+Math.round(result[x].item_price)+'</p>'
	cart_product_list+='<p>seller_id:'+result[x].seller_id+'</p>'
//	cart_product_list+='<p>Discount:'+result[x].buyer_id+'</p>'

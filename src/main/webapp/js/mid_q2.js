...
console.log(document.getElementById('place_order').innerHTML);
 ...
 price=[];
  $.ajax({
        type :"GET",
       url: "http://localhost:9000/EC/webapi/itemService/showAllItems",
       async: false,
       headers: {
...
//             document.getElementById('button_dropdown').innerHTML=sortDropDown;
             cart_product_list='<ul class="list-group">';
              for(i in result)
              {
              	if(parseInt(result[i].price,10)<parseInt(document.getElementById('To').value ))
         		{
              		if(parseInt(result[i].price,10)>parseInt(document.getElementById('From').value ))
             		{
...
//	console.log(key);
    ...
    for(i in result)
    {
//    	if(parseInt(result[x].price,10)<6000)
//    		{
//    		console.log("IN PRICE RESULT");
//    		console.log(parseInt(result[x].price,10));
    			cart_list(i);/*function call*/
//    		}
    }
    cart_product_list+='</ul>';
...
//                                  '</table>'+
//                                '</span>'+
                          '<span class="col-sm-2" style="margin-left:50px"><p>'+Sno+'</p></span>'+
                                '<span class="col-sm-2" style="margin-left:50px"><p>'+result[x].barcode+'</p></span>'+
                                '<span class="col-sm-2" style="margin-left:50px"><p>'+Math.floor(result[x].price)+'</p></span>'+
                                '<span class="col-sm-2" style="margin-left:50px"><p>'+result[x].seller_name+'</p></span>'+
//                                '<span class="col-sm-3">'+

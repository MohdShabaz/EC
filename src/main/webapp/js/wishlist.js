...
//console.log(document.getElementById('place_order').innerHTML);
 ...
 price=[];
  $.ajax({
        type :"GET",
       url: "http://localhost:9000/EC/webapi/wishlistService/AllWishlistDetails",
       async: false,
       headers: {
...
//								'<span class="col-sm-2"><p><b>Rs.'+((result[x].quantity)*(result[x].old_price)).toFixed(2)+'</b></p></span>'+
                                '<span class="col-sm-2"><p><b>Rs.'+Math.round((result[x].price))+'</b></p>'+
                            //      '<p>Free</p>'+
																'</span>'+
																'</div>'+
                            '</span>'+
...
function Remove(x){
	console.log(result[x].item_id);
	  $.ajax({
	            type :"DELETE",
	            url: "http://localhost:9000/EC/webapi/wishlistService/deletewishlistItem/"+result[x].item_id,
	            dataType:'text',
	            headers: {
	                'username':sessionStorage.username?sessionStorage.getItem('username'):'',
	                'password':sessionStorage.password?sessionStorage.getItem('password'):''//sessionStorage.getItem('password')
...
function MoveToCart(x){
	console.log(result[x].item_id);
	  $.ajax({
	            type :"DELETE",
	            url: "http://localhost:9000/EC/webapi/wishlistService/movewishlistItem/"+result[x].item_id,
	            dataType:'text',
	            headers: {
	                'username':sessionStorage.username?sessionStorage.getItem('username'):'',
	                'password':sessionStorage.password?sessionStorage.getItem('password'):''//sessionStorage.getItem('password')
	            ...
	            error: function(response){
	            	alert("Sorry Out of Stock:Cannot Be added to Cart");
		             location.reload();
		            }
	          });
};
